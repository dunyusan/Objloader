...
ObjModel *
ObjLoadModel (char *memory, size_t size)
{
  char *p = NULL, *e = NULL;
  ...
  ret->TexCoordArray =
    (ObjTexCoord *) malloc (sizeof (ObjTexCoord) * ret->nTexCoord);
  ret->TriangleArray =
    (ObjTriangle *) malloc (sizeof (ObjTriangle) * ret->nTriangle);
  ...
  while (p != e)
    {
      if (memcmp (p, "v", 1) == 0)	/* or *p == 'v' */
	{
	  sscanf (p, "v %f %f %f", &ret->VertexArray[nV].x,
		  &ret->VertexArray[nV].y, &ret->VertexArray[nV].z);
	  nV++;
	}
...
size_t
ObjLoadFile (char *szFileName, char **memory)
{
  size_t bytes = 0;
